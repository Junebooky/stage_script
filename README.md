...
### Operator의 두 가지 시연 모드

**M06 추가:** 음원 시연 모드에서 `시연 넘버 → M06`을 선택하면 로컬 백엔드에 등록된 136초 원본으로 26개 자막을 자동 송출합니다. `npm run dev:backend`가 필요하며, 대신 원본 파일을 선택하면 SHA-256으로 M06을 자동 선택하고 브라우저에서만 재생합니다. 원본 파일과 분석 결과는 Git에 포함하지 않습니다. 타이밍은 실제 Groq 단어 시각을 이용한 silver reference로 청취 검수 전입니다. [분석 결과와 검증 방법](docs/m06-demo-report.md).

`/operator`는 이제 **음원 시연 모드 / 라이브 마이크 모드**를 명시적으로 분리합니다. 왼쪽은 확정 무대 자막, 오른쪽은 다음 큐 스택입니다. JSON import, ASR 공급자, 프로필, ARM/GO 수동 운용 및 진단 로그는 하단의 기본 닫힘 `엔지니어링 설정 (Advanced Settings)`에 있습니다.

- **음원 시연**: `apps/web/public/M5-2_외로운별_0420.wav`를 준비하거나 `음원 파일 선택`으로 원본 WAV를 엽니다. 기존 원본을 public으로 이동·개명할 필요가 없습니다. 설정에서 등록 원본의 로컬 서버 경로를 선택할 수도 있습니다. 파일 선택은 object URL을 이용한 브라우저 내 재생이며 업로드하지 않습니다. Play/Pause/Seek/Stop, 1.0× media clock 및 timecode를 제공하고 R001 reference 시각에 27개 canonical cue를 송출합니다. 원본 길이와 1초 이상 다르면 재생을 차단하지만 같은 길이의 다른 편집본까지 검증하는 것은 아닙니다.
- **라이브 마이크**: 관객 화면을 같은 origin에서 열고 큰 마이크 버튼을 한 번 누릅니다. 입력·ASR·에셋·관객 연결 준비가 완료되면 ARM/GO를 순서대로 진행합니다. 기존 `ShowRuntime` / `PERFORMANCE_LOCAL` matcher를 유지하며, partial 원문은 별도 마이크 패널에만 표시합니다. 브라우저 입력은 온라인 서비스 전송 가능성을 표시하며 로컬 입력으로의 자동 fallback은 없습니다.
- **모드 전환**: 이전 media playback / recognition / microphone tracks를 정리하고 새 송출 세션을 시작합니다. 재생 위치와 live cursor는 모드 간 공유하지 않습니다. Space는 수동 다음 큐, R은 라이브 재탐색 또는 음원 시각으로 복귀입니다. 입력 필드·버튼·설정 summary에 포커스가 있을 때 전역 단축키는 개입하지 않습니다.

**주의:** 새 음원 모드는 사용자가 요청한 reference 기반 연출 시연입니다. 기존 `/rehearsal`의 saved-ASR replay와는 별도 훅/경로이며 그 matcher, seek 제한, 평가 수치를 변경하지 않습니다. Reference는 라이브 matcher나 ASR 평가용 replay trigger에 들어가지 않습니다. 이 시연의 동기화 결과를 live ASR 정확도·latency로 보고하지 않습니다. Canonical 36개 중 R001 녹음에 없는 9개를 제외하는 것도 이 recording projection에 한정합니다.
...
Groq는 **녹음 파일 분석 전용**입니다. `whisper-large-v3`, 한국어, temperature 0, `verbose_json`, word+segment 시각을 요청하며 대본·cue ID·앵커 prompt는 보내지 않습니다. Free Tier용으로 원본은 전송하지 않고 **임시 16kHz mono FLAC**을 먼저 만듭니다. FFmpeg/ffprobe로 전체 디코딩·재생시간(16kHz 1샘플=0.0625ms 이내)·offset 0·25,000,000 bytes 이하를 확인한 뒤 한 번 전송합니다. Trimming/tempo change/silence removal은 없고 원본 SHA를 전후 검사합니다. FLAC은 변환된 PCM의 무손실 압축이며 리샘플링·mono 변환 자체가 원본 파형과 bit-identical한 것은 아닙니다. 초과/검증 실패/요청 실패 시 재시도·자동 fallback·임의 chunking 없이 중단하고 임시 파일을 정리합니다. 단어 confidence가 없으면 `null/unavailable`이고 segment `exp(avg_logprob)`는 `segment-logprob-derived`로 구별합니다. 이는 보정된 단어 확률이 아닙니다. [Groq STT 명세](https://console.groq.com/docs/speech-to-text).
...
매번 `.stage-data/number-analysis/run-<UUID>/`를 만들고 원본/이전 실행을 덮어쓰지 않습니다. `inspection.json`, `status.json`, `run.json`; 실제 ASR 성공 시 `observation.json`, `alignment.json`, `metrics.json`, 근거가 있으면 `candidate-profile.json`을 추가합니다. `asr-benchmark.json`과 22항목 `report.md`에는 동일 오디오 SHA, provider/model, cue coverage, 미관측/반복/전환 검토, 정규화에 채택된 단어 시각·신뢰도 유무, 배치 요청 wall time을 기록합니다. 원응답 단어 수는 `external-asr.json`의 rawResult에서 확인합니다. 인간 정답 없는 WER/CER와 live latency는 계산하지 않습니다. 외부 호출 감사는 `external-asr.json`이며 헤더·키는 저장하지 않습니다. 성공 0 / 실패 1 / 모델·키·동의 없음 2로 종료합니다. CLI 결과는 업로드 이력에 자동 합쳐지지 않습니다. 승인된 실제 M05-2 분석 1회가 성공했으며 기계 정렬 28/36, 미관측 8, 검토 큐 8이 나왔습니다. 후기 C016 배정은 오정렬 의심이며 후보 replay 개선은 없으므로 공연용으로 승인하지 않습니다. [Groq 구현·검증 현황](docs/groq-rehearsal-report.md).
