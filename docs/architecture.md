...
Groq `whisper-large-v3`는 원본을 업로드하지 않고 승인된 임시 16kHz mono FLAC만 고정 `api.groq.com/openai/v1/audio/transcriptions`에 전송합니다. `groq_transport.py`가 전체 decode, codec/rate/channel, duration 차이 1출력샘플 이내, offset 0을 검증하고 provider가 업로드 전 25,000,000 bytes 한도를 확인합니다. 한국어, temperature 0, verbose_json, word+segment timestamps만 요청하며 canonical prompt는 없습니다. 원본 수정/trimming/tempo change/silence removal은 하지 않습니다. FLAC은 리샘플링·downmix 이후 PCM의 lossless 압축입니다. 한 번의 요청만 허용하며 초과/실패 시 재시도/fallback/chunking 없이 중단합니다. 원본 SHA-256, 변환·시간·크기 검증, request ID, 배치 요청 wall time과 전체 pipeline time, 성공/실패 임시 파일 정리를 감사하되 키/헤더는 제외합니다. 이 endpoint의 원격 삭제는 앱에서 지원할 수 없습니다. [명세](https://console.groq.com/docs/speech-to-text) · [보관 정책](https://console.groq.com/docs/your-data).
